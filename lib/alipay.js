...
var default_alipay_config = {
    ...
    ,it_b_pay:"1d"
	,create_direct_pay_by_user_return_url : '/api/create_direct_pay_by_user/return_url'
	,create_direct_pay_by_user_notify_url: '/api/create_direct_pay_by_user/notify_url'
	,refund_fastpay_by_platform_pwd_notify_url : '/api/refund_fastpay_by_platform_pwd/notify_url'
	,create_partner_trade_by_buyer_notify_url: '/api/create_partner_trade_by_buyer/notify_url'
	,create_partner_trade_by_buyer_return_url: '/api/create_partner_trade_by_buyer/return_url'
	
	,trade_create_by_buyer_return_url : '/api/trade_create_by_buyer/return_url'
	,trade_create_by_buyer_notify_url: '/api/trade_create_by_buyer/notify_url'
	,trade_securitypay_notify_url: '/api/trade_securitypay/notify_url'
};
...
Alipay.prototype.route = function(app){
    ...
    app.post(this.alipay_config.trade_securitypay_notify_url, function(req, res){self.trade_mobiletrade_notify(req, res)});
}
...
Alipay.prototype.trade_mobiletrade_notify = function(req, res){
    var self = this;

    var _POST = req.body;
    console.log("mobiletrade_ " + JSON.stringify(_POST));
    //计算得出通知验证结果
    var alipayNotify = new AlipayNotify(this.alipay_config);
    //验证消息是否是支付宝发出的合法消息
    alipayNotify.verifyNotify(_POST, function(verify_result){
        if(verify_result) {//验证成功
            console.log("verfiy_result " + JSON.stringify(verify_result));
            ...
            if(trade_status  == 'WAIT_BUYER_PAY'){
                self.emit('trade_securitypay_wait_buyer_pay', _POST);
            }
            else if(trade_status == 'TRADE_FINISHED'){
                self.emit('trade_securitypay_trade_finished', _POST);
            }
...
Alipay.prototype.generalOrderNo = function() {
    var numOutput = new Number(Math.random() * 1000).toFixed(0);
    var now = new Number(new Date().getTime()/1000).toFixed(0);
    return ""+now+numOutput;
}

Alipay.prototype.generalOrderParams = function(data){
    var self = this;
    ...
    var parameter = {
        ...
        ,notify_url: encodeURIComponent(url.resolve(this.alipay_config.host, this.alipay_config.trade_securitypay_notify_url))
        ,service:"mobile.securitypay.pay"
        ,payment_type:'1' //支付类型
        ,_input_charset:this.alipay_config['input_charset'].toLowerCase().trim()
    };

    return alipaySubmit.buildRequestParaSecurePay(parameter);
