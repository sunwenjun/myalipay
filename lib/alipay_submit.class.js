...
var rsa_f = require('./alipay_rsa.function');
...
AlipaySubmit.prototype.buildRequestMysign = function(para_sort){
    ...
    var prestr = "";
    if (sign_type == "RSA") {
        prestr = core_funcs.createLinkstringQuote(para_sort);
    }
    else {
    ...
    if(sign_type == "MD5"){
        ...
        mysign = rsa_f.rsaSign(prestr, this.alipay_config['key']);
    }
    else{
        mysign = "";
    }

   // console.log("prestr " + prestr + "sign: " + mysign);
    return mysign;
}
...
AlipaySubmit.prototype.buildRequestParaSecurePay = function(para_temp){
    //除去待签名参数数组中的空值和签名参数
    var para_filter = core_funcs.paraFilter(para_temp);
    //对待签名参数数组排序
    var para_sort = null;
...
//    para_sort['sign_type'] = this.alipay_config['sign_type'].trim().toUpperCase();
    ...
    return core_funcs.createLinkstringQuote(para_sort)+"&sign=\""+encodeURIComponent(mysign)+"\"&sign_type=\"RSA\"";
}
